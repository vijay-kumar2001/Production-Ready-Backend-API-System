...
//these will also have automatically generated ._id but that is for sessions and userId is ._id generated by mongodb for user doc ie for every new user unique ._id is generated in users collection and that is being stored here as userId
...
export const sessionModel = {
    ...
    async findByUserId(userId){
        return Session.find({userId}).lean(); //returns array of all documents where {userId:userId} , there is no findMany function in mongoose
    },
    async delete(sessionId) {
        return Session.deleteOne({ sessionId });
    },
    async deleteByUserId(userId){
        return Session.deleteMany({userId}); // it has single value and refreshTokenModel had array of values that is why that used template literal like thing 
    }
};
